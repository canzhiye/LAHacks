...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        ...
        default: {
            NSLog(@"BEACONS %@", self.detectedBeacons);
...
- (void)turnOnRanging
{
    ...
    //NSLog(@"%@",self.beaconRegion.proximityUUID.UUIDString);
    //self.detectedBeacons.count
...
- (void)locationManager:(CLLocationManager *)manager
    ...
    if (filteredBeacons.count == 0) {
        ...
        //BEACON FOUND HERE
        for (int i = 0; i < filteredBeacons.count; i++) {
            CLBeacon *beacon = filteredBeacons[i];
            NSLog(@"beacon: %@", beacon);
            
            Firebase *f = [[Firebase alloc]initWithUrl:@"https://fastcheck.firebaseio.com/"];
            [[f childByAppendingPath:[NSString stringWithFormat:@"%@",beacon.major]] setValue:@"stuff"];
        }
    }
